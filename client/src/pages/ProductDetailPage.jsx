...
const ProductDetailPage = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <Link to="/products" className="inline-flex items-center gap-1 text-sm text-surface-500 hover:text-primary-600 mb-6 no-underline">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
          <div className="aspect-square rounded-2xl overflow-hidden bg-surface-100 mb-4">
            {product.images?.[selectedImage]?.url ? (
              ...
              <div className="w-full h-full flex items-center justify-center"><ShoppingBag className="w-16 h-16 text-surface-300" /></div>
            )}
          </div>
            ...
            <div className="flex gap-3">
              {product.images.map((img, i) => (
                <button
                  ...
                  className={`w-16 h-16 rounded-xl overflow-hidden border-2 cursor-pointer ${selectedImage === i ? 'border-primary-600' : 'border-surface-200'}`}
                >
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-5">
          ...
          <h1 className="text-2xl md:text-3xl font-bold text-surface-900">{product.name}</h1>
          <div className="flex items-center gap-2">
            <StarRating rating={product.ratings} size={18} />
            <span className="text-sm text-surface-500">({product.numOfReviews} reviews)</span>
          </div>
          ...
          {product.stock > 0 && (
            <div className="flex items-center gap-4 pt-4">
              <div className="flex items-center gap-2 bg-surface-100 rounded-xl">
                <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="p-3 hover:bg-surface-200 rounded-l-xl cursor-pointer"><Minus className="w-4 h-4" /></button>
                <span className="w-12 text-center font-semibold">{quantity}</span>
                <button onClick={() => setQuantity(Math.min(product.stock, quantity + 1))} className="p-3 hover:bg-surface-200 rounded-r-xl cursor-pointer"><Plus className="w-4 h-4" /></button>
              </div>
