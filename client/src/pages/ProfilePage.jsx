            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              {loading ? <Loader /> : orders.length === 0 ? (
                <div className="text-center py-16 bg-white rounded-2xl border border-surface-200">
                  ...
                  <p className="text-sm text-surface-400 mt-1">Start shopping to see your orders here</p>
                ...
                <div className="space-y-3">
                  {orders.map((order) => (
                    <Link
                      key={order._id}
                      to={`/orders/${order._id}`}
                      className="block bg-white rounded-2xl border border-surface-200 p-5 hover:border-primary-300 hover:shadow-sm transition-all no-underline"
                    >
                      <div className="flex items-center justify-between mb-3">
                        <div>
                          <p className="font-semibold text-surface-800">Order #{order._id?.slice(-8).toUpperCase()}</p>
                          <p className="text-xs text-surface-400">{new Date(order.createdAt).toLocaleDateString('en-PK', { year: 'numeric', month: 'short', day: 'numeric' })}</p>
                        </div>
                ...
                <div className="flex items-center justify-between mb-6">
                  ...
                  {!isEditing && (
                    <button
                      ...
                      className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-primary-600 bg-primary-50 rounded-xl hover:bg-primary-100 transition-colors cursor-pointer"
                    >
                  ...
                  <form onSubmit={handleSubmit(onProfileSubmit)} className="space-y-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        value={user?.email}
                        disabled
                        className="w-full px-4 py-2.5 border border-surface-200 rounded-xl bg-surface-50 text-surface-400 text-sm"
                      />
                    ...
                    <div className="flex gap-3 pt-2">
                      ...
                      <button
                        ...
                        className="flex items-center gap-2 px-5 py-2.5 border border-surface-200 text-surface-600 rounded-xl font-medium hover:bg-surface-50 transition-colors cursor-pointer text-sm"
                      >
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <p className="text-surface-800 font-medium mt-1">{user?.name}</p>
                      </div>
                      <div>
                        <label className="text-sm font-medium text-surface-500">Email</label>
                        <p className="text-surface-800 font-medium mt-1">{user?.email}</p>
                      </div>
                      <div>
                        <label className="text-sm font-medium text-surface-500">Phone</label>
                        <p className="text-surface-800 font-medium mt-1">{user?.phone || 'Not set'}</p>
                      </div>
                      <div>
                        <label className="text-sm font-medium text-surface-500">Role</label>
                        <p className="text-surface-800 font-medium mt-1 capitalize">{user?.role}</p>
                      </div>
                    </div>
                    {user?.address && (user.address.street || user.address.city) && (
                      <div className="pt-4 border-t border-surface-200">
                        <label className="text-sm font-medium text-surface-500 block mb-2">Address</label>
                    ...
                    <div className="pt-4 border-t border-surface-200">
                      <label className="text-sm font-medium text-surface-500 block mb-2">Member Since</label>
